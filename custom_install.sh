#!/bin/bash
#
# This script installs Sway on a pre-installed arch linux
# A user with sudo permissions and a live network connection is needed
#

# Set up logging
exec 1> >(tee "OS_config_out")
exec 2> >(tee "OS_config_err")

log(){
    echo "*********** $1 ***********"
    now=$(date +"%T")
    echo "$now $1" >> ~/OS_config_log
}

check(){
    if [ "$1" != 0 ]; then
	echo "$2 error : $1" | tee -a ~/OS_config_log
	exit 1
    fi
}
